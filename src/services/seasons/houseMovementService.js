...
export function subscribeToRosterLocks(leagueId, onUpdate, onError) {
  if (!leagueId) {
    onUpdate?.([]);
    return () => {};
  }
  return onSnapshot(
    query(collection(db, "leagueRosterLocks"), where("leagueId", "==", leagueId)),
...
export function subscribeToHouseAssignmentHistory(leagueId, onUpdate, onError) {
  ...
  return onSnapshot(
    query(collection(db, "leagueHouseAssignmentHistory"), where("leagueId", "==", leagueId)),
    (snapshot) => onUpdate?.(mapSnapshot(snapshot)),
    onError,
  );
}
...
export async function calculateWeeklyHouseBalance({
  ...
  return runTransaction(db, async (transaction) => {
    ...
    const auditReference = addAuditWrite(transaction, {
      ...
      details: {
        weekKey,
        activeMemberCount: publicResult.activeMemberCount,
        responseCount: publicResult.responseCount,
