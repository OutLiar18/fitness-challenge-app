...
This document defines the security philosophy and access model used throughout Champions Legacy Challenge.
...
Champions Legacy Challenge follows the principle:
...
## Constrained Player Profile Updates

Players may update only their display name, approved Legacy Avatar identifier and profile update timestamp. Firestore rules compare changed fields so role, email, ownership, team and join-date data cannot be modified by the client.

Avatar selections reference a local approved catalogue. They do not contain arbitrary URLs or uploaded media.
