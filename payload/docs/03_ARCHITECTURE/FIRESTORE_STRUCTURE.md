...
# Progression Persistence Status — v0.7.0
...
This document defines how the Champions Legacy Challenge data model is implemented within Firestore.
...
User profile documents now include:

- `avatarId` — approved local Legacy Avatar identifier.
- `profileUpdatedAt` — server timestamp for the latest allowed profile edit.

No image file or external image URL is stored.
