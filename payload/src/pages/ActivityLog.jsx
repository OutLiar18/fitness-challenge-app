...
export default function ActivityLog() {
  ...
  const requestedCategory = getSafeCategoryId(searchParams.get("category"));
  const [selectedDate, setSelectedDate] = useState(() =>
    normalizeChallengeDate(new Date()),
  );
  const [categoryId, setCategoryId] = useState(requestedCategory);
  const [formData, setFormData] = useState(() =>
    getInitialFormData(requestedCategory),
  );
  ...
  useEffect(() => {
    const nextCategory = getSafeCategoryId(searchParams.get("category"));
  ...
  function handleCategorySelect(nextCategoryId) {
    ...
    setFormData(getInitialFormData(safeCategoryId));
    setFormErrors([]);
    setSearchParams({ category: safeCategoryId }, { replace: true });
  }
  ...
  async function handleSaveEntry() {
    ...
    try {
      ...
      const nextCategory = getSafeCategoryId(result.nextCategory ?? categoryId);
      setCategoryId(nextCategory);
      setFormData(getInitialFormData(nextCategory));
  ...
  return (
    <div className="activity-page page-stack">
      <PageHeader
        ...
        description="Record the facts once, then let Champions Legacy calculate points, goals, XP and progress centrally."
        icon="✍️"
      />
      ...
      <div className="activity-page__hint card">
        ...
        <p>
          <strong>Honesty bonus:</strong> not an actual bonus—just the reason your
          progress remains useful.
        </p>
      </div>
