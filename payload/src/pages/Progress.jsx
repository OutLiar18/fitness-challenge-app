...
function formatPace(secondsPerKm) {
  ...
  return `${Math.floor(totalSeconds / 60)}:${String(totalSeconds % 60).padStart(
    2,
    "0",
  )}/km`;
}
...
export default function Progress() {
  const { profile, user, entries, loading } = usePlayerData();
  ...
  return (
    <div className="progress-page page-stack">
      <PageHeader
        ...
        description="XP, streaks, achievements and records are derived from your factual activity history—not stored as hidden scores."
        icon="📈"
      />
      ...
      <section className="progress-hero card">
        <div className="progress-hero__content">
          ...
          <h2>{xp.title}</h2>
          <p>
            Your strongest opponent remains the version of you who said “maybe
            tomorrow.”
          </p>
        </div>

        <div className="progress-hero__level" aria-label={`Personal level ${xp.level}`}>
          <span>Level</span>
          <strong>{xp.level}</strong>
          <small>{formatWholeNumber(xp.totalXp)} total XP</small>
        </div>
      </section>

      <section className="progress-summary-grid" aria-label="Progress summary">
        {[
          ["⚡", formatWholeNumber(xp.totalXp), "Total XP"],
          ["⭐", formatWholeNumber(score.totalPoints), "Total points"],
      ...
      <div className="progress-primary-grid">
        <section className="progress-section card" aria-labelledby="xp-title">
        ...
        <section className="progress-section card" aria-labelledby="streak-title">
          ...
          <div className="progress-streak-grid">
            {[
              ...
              ["🛡️", streak.shieldAvailable, "Shield available"],
            ].map(([icon, value, label]) => (
              <article key={label}>
                <span aria-hidden="true">{icon}</span>
                <strong>{value}</strong>
          ...
          <div className="progress-streak-note">
            <span aria-hidden="true">{streak.shieldAvailable > 0 ? "🛡️" : "🌱"}</span>
            <p>
          ...
          {streak.nextMilestone && (
            <div className="progress-next-milestone">
              ...
              <small>
                +{streak.nextMilestone.points} points · +{streak.nextMilestone.xp}{" "}
                XP
              </small>
            </div>
          )}
          ...
          {achievements.achievements.map((achievement) => (
            ...
            >
              ...
              <div>
                ...
                <small>{achievement.unlocked ? "Unlocked" : "Not unlocked yet"}</small>
              </div>
            </article>
          ))}
        </div>
