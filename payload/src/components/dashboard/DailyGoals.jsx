...
function getEntryRequirement(goal) {
  ...
  return `${goal.entryCount} of ${goal.minimumEntries} ${pluralize(
    goal.minimumEntries,
    "run",
    "runs",
  )} logged`;
  ...
  return (
    <section className="daily-goals card" aria-labelledby="dashboard-goals-title">
      <div className="daily-goals__header">
        ...
        <span>
          <strong>{completed}</strong> of {goals.length} complete
        </span>
      </div>
      ...
      >
        {goals.map((goal) => {
          ...
          const current = formatMeasurement(goal.current, goal.unit);
          const target = formatMeasurement(goal.goal, goal.unit);
          const bonus = formatPoints(goal.bonusPoints);
          ...
          return (
            <button
              ...
              aria-label={`${goal.name}: ${current} of ${target}. ${goal.percentage} percent complete. ${
                goal.completed
                  ? `${bonus} earned.`
                  : `${bonus} available on completion.`
            ...
            >
              ...
              <span className="goal-card__footer">
                ...
                <span>
                  {goal.completed ? `${bonus} earned ✓` : `${bonus} at goal`}
                </span>
              </span>
