...
} from "../src/services/announcements/announcementService.js";
import {
...
test("Announcement helpers sort, filter and calculate unread status", () => {
  ...
  const firstDate = announcements[0].publishedAt;
  const secondDate = announcements[1].publishedAt;

  assert.equal(firstDate >= secondDate, true);
